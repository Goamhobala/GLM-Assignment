---
title: "STA304XS Assignment 3"
author: "Saurav Sathnarayan, Jing Yeh"
date: "2025-10-19"
output: pdf_document
---
...
```{r}
dat = read.table("~/Desktop/STA304XS/STA304XS assignment 3/Assignment_Disease.txt", header = TRUE)
attach(dat)
Y <- dat$DiseaseStatus
...
```{r}
library(glmnet)
fit_glmnet <- glmnet(X, Y, family = "binomial", alpha = 0, lambda = 1)
fit_glmnet$beta
```
...
```{r}
train_pred <- predict(fit, X, type = "class")
val_pred <- predict(cv_fit, X, s = "lambda.min", type = "class")
train_acc <- mean(train_pred == Y)
val_acc <- mean(val_pred == Y)
```



```{r}
overfit_pred <- predict(fit, X, s = min(fit$lambda), type = "class")
overfit_acc <- mean(overfit_pred == Y)

# Ridge model (optimal lambda)
ridge_pred <- predict(cv_fit, X, s = "lambda.min", type = "class")
ridge_acc <- mean(ridge_pred == Y)
```
